...
if isempty(strobeInfo)
    ...
    % strobeInfo.trial_start_samp(end) = max([strobeInfo.trial_start_samp(end),numDataPoints]);  %Don't think this is needed
...
% %     %Old code that takes all timepoints of data out at once, runs into RAM
% %     %issues with larger files - lines 215-278
...
%         fileData = readTrodesExtractedDataFileWithTime( [dataPaths.input_file_path , envInfo.rec_file_name, '.raw\' envInfo.rec_file_name '.raw_nt' num2str(SigQuality(curr_index).channels(ch)) 'ch1.dat'],1,100000000);
% 
%         if ch == 1
...
%                 spike_times{ch}     = [spike_times{ch}; uint32(threshold_crossings)-1];
...
%     end
    ...
    %Loops through time by strobeInfo using start and stop from
    %strobeInfo.trial_start_samp & strobeInfo.trial_end_samp
    %Not actual behavioral trials, just consecutive chunks of data
        ...
        if strobeInfo.trial_end_samp(tr) <= numDataPoints
            for ch = 1:length(SigQuality(iArr).channels)
                fileData = readTrodesExtractedDataFileWithTime( [dataPaths.input_file_path , envInfo.rec_file_name, '.raw\' envInfo.rec_file_name '.raw_nt' num2str(SigQuality(curr_index).channels(ch)) 'ch1.dat'],strobeInfo.trial_start_samp(tr),strobeInfo.trial_end_samp(tr));
            ...
            for ch = 1:length(envInfo.channels_to_read_by_array{iArr})
                if abs(ChMedians(ch)) > 0
                    ...
                    if exist('trialMargin','var')
                        ...
                        elseif tr == length(strobeInfo.TrialIDs) %if last chunk of data, no trialMargin at end
    ...
    %Old code for reading ns5 files from back in Rochester
%     for tr = 1:length(strobeInfo.TrialIDs)
%         if strobeInfo.trial_end_samp(tr) < numDataPoints
%             % Time string
