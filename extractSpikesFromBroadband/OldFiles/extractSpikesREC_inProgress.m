...
function extractSpikesREC_inProgress(dataPaths, envInfo, strobeInfo, filtInfo)
...
for iFile = unique_files
    ...
    tic
    parfor iArr = find(envInfo.array_to_fileNum==iFile)  %1:length(envInfo.channels_to_read_by_array)
        ...
        [spike_times{iArr}, spike_snippets{iArr}] = find_waveform_thresholds(dataPaths, envInfo, strobeInfo, numDataPoints, iArr, filtInfo, ChMedians, WhtMat);
    end
    toc
    ...
    for iArr = find(envInfo.array_to_fileNum==iFile)
